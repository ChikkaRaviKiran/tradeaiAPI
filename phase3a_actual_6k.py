#!/usr/bin/env python3
"""Phase-3a ACTUAL — original 8 strategies, ±₹6,000 rupee gates (TP+SL).
...
def main():
    ...
    print(f"Gates:       TP=+₹{TP_RS:,.0f}  SL=−₹{abs(SL_RS):,.0f}  + SL%-of-credit safety")
    ...
    print("══════════ TOP-5 per (weekday × index) — PHASE 3a actual, ±₹6k ══════════")
    ...
    print("\n══════════ FINAL RECOMMENDATION — PHASE 3a (±₹6k, one index/day) ══════════")
    best_wd = {}
    for s in survivors:
        wd = s["wd"]
