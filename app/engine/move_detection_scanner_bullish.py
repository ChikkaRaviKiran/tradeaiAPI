"""Move Detection Scanner — BULLISH variant (Production v2 AGGRESSIVE tier).

Mirror of `move_detection_scanner.py` but trades the BULLISH side:
expansion-up → hold above body-mid → continuation-up.  Buys ATM CE
instead of ATM PE; exits flip the comparisons (`close < body_mid`,
`close < vwap`, swing-low trail).

Designed to coexist with the bearish scanner — runs independently with
its own daily / weekly state.  Use the new module so the existing
bearish scanner is **not** modified.

AGGRESSIVE tier filters (mirror of bearish):
  - Bullish direction only (skip all bearish)
...
Exit rules (mirror, flipped):
  1. Structure break — close < expansion body midpoint
  2. VWAP break — close < VWAP × 0.999
  3. Trailing stop — after 10 candles in profit, trail using 5-candle
     swing LOW − buffer
  4. Max hold — 120 candles (~2 hours)
  5. End of day — 15:20 force close
  6. Premium target — option LTP up ≥ ₹20 from entry
"""
...
# ── AGGRESSIVE tier parameters (mirror of bearish, direction flipped) ──
MIN_CONFIDENCE_BULLISH = 70     # CONSERVATIVE filter set (see SELECT_FILTER below)
MIN_CONFIDENCE_BEARISH = 999    # Effectively skip all bearish
EARLIEST_CANDLE = 45            # ~10:00 AM
DEADLINE_CANDLE = 135           # ~11:30 AM
TRAIL_LOOKBACK = 5
TRAIL_BUFFER_PCT = 0.02         # 0.02% buffer on trailing stop
OPTION_LEVERAGE = 55            # Approximate ATM CE leverage
...
# CONSERVATIVE filter (backtested 2025-10 → 2026-05, 144 days):
#   11 trades, 8W/3L (72.7% win), +443 pts, MaxDD only −6 pts, RR 27.4
# Drops a bullish signal when ANY of these is true:
#   - the entry was improved by a micro-pullback
#   - either ema_aligned or vwap_aligned is True (trend-chasing dies fast)
#   - expansion_ratio >= 2.0 (over-extended = exhaustion)
REJECT_IF_PULLBACK = True
REJECT_IF_EMA_ALIGNED = True
REJECT_IF_VWAP_ALIGNED = True
MAX_EXPANSION_RATIO = 2.0       # strictly less-than is required

SCANNER_TAG = "MOVE-DET-BULL"   # Telegram message prefix
EXEC_SETTINGS_KEY = "move_det_bull"


class ActiveTrade:
    """Tracks an active bullish Move Detection signal from entry to exit."""
    ...
    def __init__(
        ...
        # Best price = HIGHEST price achieved (bullish: higher is better)
...
class MoveDetectionScannerBullish:
    """Production v2 AGGRESSIVE scanner — BULLISH variant."""
    ...
    def __init__(
        ...
        self._last_trade_week: Optional[int] = None
        self._today_str: str = ""
        self._expiry: str = ""
    ...
    async def run_cycle(
        ...
        cfg = exec_settings.load(EXEC_SETTINGS_KEY)
        ...
        if self._today_str != today_str:
            if self._today_str:
                logger.info(
                    "[MoveDetBull] Day rollover detected: %s → %s. Resetting daily state.",
        ...
        if self._expiry_date and now.date() == self._expiry_date:
            ...
            if not getattr(self, "_expiry_skip_notified", False):
                ...
                logger.info(
                    "[MoveDetBull] Day %s SKIPPED: options expiry day (expiry=%s)",
                ...
                try:
                    ...
                    logger.exception("[MoveDetBull] Failed to send expiry-skip Telegram")
            return
        ...
        if (
            ...
            if not tradeable or vwap_trend == "mild_chop":
                ...
                logger.info(
                    "[MoveDetBull] Day %s SKIPPED: %s at %s (candles=%d, VWAP crosses=%d, trend=%s)",
                    self._today_str, reason, now.strftime("%H:%M"), candle_count,
                ...
                logger.info(
                    "[MoveDetBull] Day %s TRADEABLE at %s: trend=%s, candles=%d, VWAP crosses=%d, direction=%.2f%%",
                    self._today_str, now.strftime("%H:%M"), vwap_trend, candle_count,
                ...
                await self.alert_manager.telegram.send(
                    ...
                    f"Scanning for bullish signals (AGGRESSIVE)..."
                )
        ...
        if candle_count > DEADLINE_CANDLE:
            if not self._signal_found_today:
                self._signal_found_today = True
                logger.info("[MoveDetBull] Past deadline (candle %d > %d) — no signal today", candle_count, DEADLINE_CANDLE)
                await self.alert_manager.telegram.send(
                    f"⏰ {SCANNER_TAG} — Deadline Passed\n"
                    f"Date: {self._today_str}\n"
                    f"No qualifying bullish signal by 11:30 AM.\n"
                    f"Done for today."
                )
            return
        ...
        trades = select_production_trade(
            df_today, moves,
            min_confidence=MIN_CONFIDENCE_BEARISH,       # bearish disabled
            min_confidence_bullish=MIN_CONFIDENCE_BULLISH,
            earliest_candle=EARLIEST_CANDLE,
            deadline_candle=DEADLINE_CANDLE,
            max_trades=5,    # allow several candidates so the CONSERVATIVE
                             # post-filter can pick the first that passes
        )
        if not trades:
            return

        trades = [t for t in trades if t["direction"] == "bullish"]
        if not trades:
            return

        # ── CONSERVATIVE post-filter (locked from backtest) ───────────
        trade = None
        for cand in trades:
            if REJECT_IF_PULLBACK and cand.get("pullback") is not None:
                continue
            if REJECT_IF_EMA_ALIGNED and cand.get("ema_aligned", False):
                continue
            if REJECT_IF_VWAP_ALIGNED and cand.get("vwap_aligned", False):
                continue
            if cand["expansion_ratio"] >= MAX_EXPANSION_RATIO:
                continue
            trade = cand
            break
        if trade is None:
        ...
        logger.info(
            "[MoveDetBull] SIGNAL DETECTED: %s conf=%d exp_ratio=%.2f ema=%s vwap=%s",
        ...
        if self._pre_entry_hook is not None:
            try:
                ok = await self._pre_entry_hook("move_det_bull", instrument, df_today)
                if not ok:
                    logger.warning("[MoveDetBull] Pre-entry hook blocked entry")
                    return
            except Exception:
                logger.exception("[MoveDetBull] Pre-entry hook failed")
                return

        self._signal_found_today = True
        spot_price = float(df_today.iloc[-1]["close"])
        strike = instrument.nearest_strike(spot_price, "CE")
        ...
        if self._expiry:
            option_symbol = instrument.build_option_symbol(self._expiry, strike, "CE")
            quote = await self._fetch_option_quote(instrument, strike, "CE")
            if quote:
        ...
        # Reuse the same `move_det_*` settings knobs (funds cap / buffer /
        # max lots / live_execution flag) — the bullish scanner does not
        # need an independent budget for the first rollout.
        live_exec = (
        ...
        # Stop-loss is BELOW entry for bullish
        sl_points = round(entry_price - body_mid, 2)
        ...
        logger.info("[MoveDetBull] Entry alert sent: %s %d CE @ ₹%.2f", option_symbol, strike, option_ltp)

        alert = AlertItem(
            id=str(uuid.uuid4())[:8],
            alert_type="signal",
            title=f"{SCANNER_TAG} SIGNAL — NIFTY {int(strike)} CE",
            message=msg,
            timestamp=now,
        )
    ...
    async def _check_exit(
        ...
        # Bullish: track HIGHEST price reached
        ...
        # Exit 0: Premium target — option LTP up ≥ ₹20
        if trade.option_entry_price > 0 and trade.option_symbol and self._expiry:
            quote = await self._fetch_option_quote(instrument, trade.strike_price, "CE")
            if quote:
        ...
        if not exit_reason and trade.candles_in_trade >= 10 and trade.best_price > trade.entry_price:
            ...
            if lookback_start < candle_count:
                swing_low = float(df_today.iloc[lookback_start:candle_count]["low"].min())
                trail_stop = swing_low * (1 - TRAIL_BUFFER_PCT / 100)
        ...
        if trade.live_executed and trade.lots > 0 and trade.symboltoken:
            ...
            if not ok:
                logger.error(
                    "[MoveDetBull] Exit order failed for %s reason=%s err=%s",
                    trade.option_symbol, exit_reason, err,
                )

        # PnL: bullish profits when price RISES
        index_pnl_pct = (exit_price - trade.entry_price) / trade.entry_price * 100
        index_pnl_pts = exit_price - trade.entry_price
        ...
        msg = (
            ...
            f"  Best price: {trade.best_price:.2f} ({trade.best_price - trade.entry_price:+.2f} pts)\n"
            f"\n"
            f"🎯 OPTION (CALL)\n"
        ...
        await self.alert_manager.telegram.send(msg)
        logger.info(
            "[MoveDetBull] EXIT %s: %s, PnL=%+.2f pts (%+.3f%%), option est=%+.1f%%",
    ...
    async def _place_exit_order(
        ...
        qty = max(1, int(trade.lots)) * max(1, int(trade.lot_size))
        return await self._place_order(
            instrument=instrument, trade=trade, side="SELL",
            quantity=qty, tag=f"mdb_exit_{reason}",
        )
    ...
    async def _place_order(
        ...
        *,
        ...
        if self.broker is None:
            logger.error("[MoveDetBull] No broker configured — cannot place %s order", side)
        ...
        try:
            ...
            logger.exception(
                "[MoveDetBull] Broker order exception side=%s symbol=%s tag=%s",
        ...
        if not resp or resp.status == OrderStatus.REJECTED:
            ...
            logger.error(
                "[MoveDetBull] Order rejected side=%s symbol=%s tag=%s msg=%s",
                side, trade.option_symbol, tag, msg,
            )
            return False, getattr(resp, "order_id", "") or "", str(msg)
        order_id = resp.order_id or ""
        logger.info(
            "[MoveDetBull] %s order placed via %s: %s qty=%d id=%s tag=%s",
    ...
    async def _fetch_option_quote(
        ...
        if not token_info:
            logger.warning("[MoveDetBull] Token not found for %s", symbol)
            return None
        try:
            ...
            logger.warning("[MoveDetBull] Quote fetch timed out for %s", symbol)
            return None
        except Exception:
            logger.exception("[MoveDetBull] Error fetching quote for %s", symbol)
            return None
