...
class Orchestrator:
    ...
    async def _pre_signal_handoff(
        ...
        Honors the global `priority_handoff_settings.enabled` flag:
          - enabled=True (default): force-close ATM/MoveDet/PDH-PDL as needed.
          - enabled=False: BLOCK the new entry if any of the three priority
            scanners or ATM is already in a trade (no rollover).

        Always closes ATL (ATM Straddle) before any MoveDet/PDH-PDL entry
        when handoff is enabled. For `source == "move_det_bull"` we also
