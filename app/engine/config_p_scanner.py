"""Config P Production Scanner — Bearish momentum detection with Telegram alerts.

This is the ONLY active signal detection system. All other strategies are disabled.

Config P filters (backtested: 16 trades, 56% win, +98.8% option PnL over 125 days):
  - Bearish direction only (skip all bullish)
  - EMA aligned required (EMA9 < EMA20)
  - Skip mild_chop days (VWAP crosses 5-8)
  - No pullback entry — aggressive entry at expansion break
  - confirmed_trend_start only, confidence >= 70
  - Entry window: candle 45-135 (~10:00-11:30 AM)
  - Weekly filter: max 1 trade per week (first signal)

Exit rules:
  1. Structure break — close breaks above expansion body midpoint (fast stop)
  2. VWAP break — close crosses above VWAP
  3. Trailing stop — after 10 candles in profit, trail using 5-candle swing high + buffer
  4. End of day — exit at 15:20 if still open
"""
...
from app.core.instruments import InstrumentConfig, NIFTY
...
# ── Config P Parameters (LOCKED — matches backtest exactly) ──────────────
MIN_CONFIDENCE = 70
...
class ActiveTrade:
    """Tracks an active Config P signal from entry through exit."""
    ...
    def __init__(
        ...
        # Exit tracking
        self.best_price = entry_price  # Lowest price seen (bearish = lower is better)
...
class ConfigPScanner:
    """Production Config P scanner — runs each minute during market hours.

    Lifecycle:
      1. At 10:00 AM (candle 45): assess day quality
      2. Each cycle 10:00-11:30: detect moves, apply Config P filters
    ...
    def reset_daily(self) -> None:
        ...
        # Don't reset _last_trade_week — that persists across days for weekly filter
    ...
    async def run_cycle(
        ...
        """Run one Config P scan cycle. Called every 60s by orchestrator."""
        ...
        # ── Already found a signal or traded this week — skip ────────
        ...
        if (
            ...
            # Config P: skip choppy AND mild_chop days
            if not tradeable or vwap_trend == "mild_chop":
                ...
                await self.alert_manager.telegram.send(
                    ...
                    f"Scanning for bearish signals..."
                )
        ...
        if candle_count > DEADLINE_CANDLE:
            if not self._signal_found_today:
                ...
                logger.info("[ConfigP] Past deadline (candle %d > %d) — no signal today", candle_count, DEADLINE_CANDLE)
                await self.alert_manager.telegram.send(
                    f"⏰ CONFIG P — Deadline Passed\n"
                    f"Date: {self._today_str}\n"
        ...
        # ── Detect moves using real-time detect_move() ───────────────
        move = detect_move(df_today)

        if move is None:
            return

        # ── Apply Config P filters ───────────────────────────────────
        ...
        if move["confidence"] < MIN_CONFIDENCE:
            return
        if not move.get("ema_aligned", False):
            return

        # Check candle index is in window
        exp_idx = move.get("expansion_idx", 0)
        if exp_idx < EARLIEST_CANDLE or exp_idx > DEADLINE_CANDLE:
            return

        # ── SIGNAL DETECTED — Get option details ────────────────────
        logger.info(
            "[ConfigP] SIGNAL DETECTED: %s conf=%d exp_ratio=%.2f ema=%s vwap=%s",
            move["direction"], move["confidence"], move["expansion_ratio"],
            move.get("ema_aligned"), move.get("vwap_aligned"),
        )
        ...
        self._active_trade = ActiveTrade(
            ...
            expansion_ratio=move["expansion_ratio"],
            ema_aligned=move.get("ema_aligned", False),
            vwap_aligned=move.get("vwap_aligned", False),
        ...
        # Structure break (SL) = body_mid for bearish
        ...
        msg = (
            ...
            f"\n"
            f"📈 SIGNAL DETAILS\n"
            f"  Confidence: {move['confidence']}/100\n"
            f"  Expansion: {move['expansion_ratio']:.1f}×\n"
            f"  Hold candles: {move['hold_candles']}\n"
            f"  EMA aligned: {'✅' if move.get('ema_aligned') else '❌'}\n"
            f"  VWAP aligned: {'✅' if move.get('vwap_aligned') else '❌'}\n"
        ...
        logger.info("[ConfigP] Entry alert sent: %s %d PE @ ₹%.2f", option_symbol, strike, option_ltp)

        # Also store in UI alert store
        alert = AlertItem(
            id=str(uuid.uuid4())[:8],
            alert_type="signal",
            title=f"CONFIG P SIGNAL — NIFTY {int(strike)} PE",
            message=msg,
            timestamp=now,
        )
    ...
    async def _check_exit(
        ...
        if not exit_reason and trade.candles_in_trade >= 10 and trade.best_price < trade.entry_price:
            # Calculate swing high of last N candles
        ...
        msg = (
            f"{emoji} CONFIG P — EXIT {'WIN' if is_win else 'LOSS'}\n"
            f"{'='*35}\n"
            f"\n"
        ...
        await self.alert_manager.telegram.send(msg)
        logger.info(
            "[ConfigP] EXIT %s: %s, PnL=%+.2f pts (%+.3f%%), option est=%+.1f%%",
        ...
        alert = AlertItem(
            id=str(uuid.uuid4())[:8],
            alert_type="exit",
            title=f"CONFIG P {'WIN' if is_win else 'LOSS'} — {exit_reason}",
            message=msg,
            timestamp=now,
        )
        await self.alert_manager.record(alert)
    ...
    async def force_close(self, df_today: pd.DataFrame, instrument: InstrumentConfig) -> None:
        ...
        if self._active_trade and not self._active_trade.exited:
            now = datetime.now(IST)
            await self._check_exit(df_today, instrument, now)
            # If still not exited (edge case), force it
    ...
    async def _fetch_option_quote(
        ...
        if not token_info:
            logger.warning("[ConfigP] Token not found for %s", symbol)
            return None
        try:
            ...
            logger.warning("[ConfigP] Quote fetch timed out for %s", symbol)
            return None
        except Exception:
            logger.exception("[ConfigP] Error fetching quote for %s", symbol)
            return None
