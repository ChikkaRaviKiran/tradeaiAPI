...
def _build_preview(body: dict, chain: list[OptionsChainRow], spot: float, support: float, resistance: float, expiry: str) -> dict:
    ...
    for side, option_type, key in STRATEGIES[strategy]["legs"]:
        ...
        premium = (row.call_ltp if option_type == "CE" else row.put_ltp) if row else None
        if premium is None: raise HTTPException(503, f"No live premium for {strike} {option_type}")
        legs.append({"side": side, "option_type": option_type, "strike": strike, "premium": round(float(premium), 2),
                     "quantity": lots * int(instrument.lot_size), "symbol": instrument.build_option_symbol(expiry, strike, option_type)})
    result = _metrics(strategy, legs, spot, int(instrument.lot_size), lots)
    scenario_prices = [support, float(_state.get("snapshots", {}).get(instrument.symbol).options_metrics.max_pain or spot) if _state.get("snapshots", {}).get(instrument.symbol) else spot, resistance]
...
def _levels(symbol: str) -> tuple[float, float, float, str, list[OptionsChainRow]]:
    ...
    return float(snap.price or snap.nifty_price), float(support), float(resistance), expiry, chain
...
def register_routes(app: FastAPI) -> None:
    ...
    async def market(symbol: str = "NIFTY"):
        symbol = symbol.upper()
        spot, support, resistance, expiry, chain = _levels(symbol)
        ...
        return {"symbol": symbol, "spot": spot, "support": support, "resistance": resistance,
                "max_pain": snap.options_metrics.max_pain, "expiry": expiry,
    ...
    async def preview(body: dict):
        spot, support, resistance, expiry, chain = _levels(str(body.get("symbol") or "NIFTY").upper())
        return _build_preview(body, chain, spot, support, resistance, expiry)
    ...
    async def place(body: dict):
        ...
        spot, support, resistance, expiry, chain = _levels(str(body.get("symbol") or "NIFTY").upper())
        preview_data = _build_preview(body, chain, spot, support, resistance, expiry)
        ...
        for leg in preview_data["legs"]:
            ...
            if hasattr(client, "_search_symbol"):
            ...
            req = OrderRequest(instrument=instrument, trading_symbol=leg["symbol"], symbol_token=token, exchange=instrument.option_exchange.value,
                ...
                underlying=instrument.symbol, expiry_date=datetime.strptime(expiry, "%d%b%y"), strike=leg["strike"], option_type=leg["option_type"], wait_for_terminal=True)
